...
\begin{tikzpicture}[font=\sffamily]
  ...
  % Title
  % --------------------------------------------------------------------------
  \node[font=\sffamily\Large\bfseries] at (11.1,10.35)
    {ESPHome high-side battery monitor};
  \node[note, text=black!65] at (11.1,9.85)
  ...
  % --------------------------------------------------------------------------
  \draw[main current]
    (0.4,7.0) to[short, o-] (1.0,7.0)
    to[fuse] (3.4,7.0)
    -- (4.9,7.0)
    to[R, l_={$R_{\mathrm{SHUNT}}$}, *-*] (7.1,7.0)
    -- (21.5,7.0)
    to[battery1] (21.5,-2.0)
    -- (0.4,-2.0) to[short, -o] (0.1,-2.0);

  \node[label text, anchor=north east] at (0.15,6.72)
    {charger/load\\positive bus};
  \node[note] at (2.2,8.0)
    {installation-rated\\main battery fuse};
  ...
  \node[label text, anchor=west] at (22.1,3.15)
    {4S LiFePO$_4$ battery\\300 Ah firmware default};
  \node[note, anchor=west, text=WarningRed] at (22.1,2.25)
    {independent BMS and\\main fuse are mandatory};
  \node[label text, anchor=west] at (0.4,-2.35)
    {battery/common negative bus};
  \draw (20.1,-2.0) node[ground] {};
  \node[note, anchor=east] at (19.75,-2.45)
  ...
  % --------------------------------------------------------------------------
  \draw[sense]
    (4.9,7.0) to[fuse, l_={$F_1$}] (4.9,5.2);
  \draw[sense]
    (7.1,7.0) to[fuse, l={$F_2$}] (7.1,5.2);
  ...
  \draw[i2c cable] (4.35,1.00) -- (18.95,1.00);
  \draw[sda] (4.35,1.12) -- (18.95,1.12);
  \draw[scl] (4.35,0.88) -- (18.95,0.88);
  ...
  % ESP32-C3 branch.
  \draw[i2c cable] (13.55,2.40) -- (13.55,1.00);
  \draw[sda] (13.43,2.40) -- (13.43,1.12);
  \draw[scl] (13.67,2.40) -- (13.67,0.88);
  \fill[SdaWire] (13.43,1.12) circle (1.6pt);
  \fill[SclWire] (13.67,0.88) circle (1.6pt);

  % SSD1306 branch.
  \draw[i2c cable] (18.55,2.40) -- (18.55,1.00);
  \draw[sda] (18.43,2.40) -- (18.43,1.12);
  \draw[scl] (18.67,2.40) -- (18.67,0.88);
  \fill[SdaWire] (18.43,1.12) circle (1.6pt);
  \fill[SclWire] (18.67,0.88) circle (1.6pt);

  \node[note, text=SdaWire, anchor=east] at (4.15,1.12) {SDA};
  \node[note, text=SclWire, anchor=east] at (4.15,0.88) {SCL};
  \node[note, text=black!65] at (11.65,0.48)
  ...
  \draw[logic power] (3.55,5.43) -- (4.72,5.43);
  ...
  \draw[logic power]
    (6.92,5.43) .. controls (6.97,5.66) and (7.23,5.66) .. (7.28,5.43);
  \draw[logic power] (7.28,5.43) -- (19.0,5.43);
  \node[note, text=LogicPower, fill=white, inner sep=1pt] at (11.9,5.72)
    {+3V3 power lane};
  ...
  \fill[LogicPower] (13.8,5.43) circle (1.8pt);
  \draw[logic power] (13.8,5.43) -- (13.8,5.2);
  \fill[LogicPower] (18.8,5.43) circle (1.8pt);
  \draw[logic power] (18.8,5.43) -- (18.8,5.2);

  % Common-negative references for the non-isolated design.
  \draw (5.0,2.4) -- (5.0,2.05) node[ground] {};
  \draw (10.7,2.4) -- (10.7,2.05) node[ground] {};
  \draw (17.2,2.4) -- (17.2,2.05) node[ground] {};

  % --------------------------------------------------------------------------
  % Protected monitor supply. Its nominal +12 V battery-bus input enters from
  % the left; the regulated output exits on the right and rises to the +3V3 lane.
  % --------------------------------------------------------------------------
  \pic (supply) at (1.3,3.55)
    {module={protected low-voltage\\supply/4.2cm/2.5cm}};
  \node[note] at (1.3,3.25)
    {nominal +12 V to regulated +3V3\\board-rated DC/DC conversion};

  % Regulated output feeds the common +3V3 lane without crossing the +12 V input.
  \draw[logic power]
    (3.4,3.0) -- (3.55,3.0) -- (3.55,5.43);
  \draw (1.0,2.3) -- (1.0,1.95) node[ground] {};

  % Separately fused bus-side input. The branch leaves the high-current path at
  % one junction, travels left of the supply, and enters the supply from the left.
  % Since the tap is on the measured side of the shunt, battery-supplied monitor
  % consumption is included in discharge current.
  \fill[MainBus] (0.75,7.0) circle (2.2pt);
  \draw[logic power]
    (0.75,7.0) -- (0.75,6.45)
    to[fuse] (0.75,5.15)
    -- (-1.15,5.15) -- (-1.15,3.9) -- (-0.8,3.9);
  \node[note, text=LogicPower, anchor=east] at (0.48,5.80)
  ...
  \ModulePort{esp-3v3}{(13.8,5.2)}{3V3}{below}{LogicPower}
  \ModulePort{esp-gnd}{(10.7,2.4)}{GND}{above}{black!70}
  \ModulePort{esp-sda}{(13.43,2.4)}{SDA}{above left}{SdaWire}
  \ModulePort{esp-scl}{(13.67,2.4)}{SCL}{above right}{SclWire}

  \ModulePort{oled-3v3}{(18.8,5.2)}{3V3}{below}{LogicPower}
  \ModulePort{oled-gnd}{(17.2,2.4)}{GND}{above}{black!70}
  \ModulePort{oled-sda}{(18.43,2.4)}{SDA}{above left}{SdaWire}
  \ModulePort{oled-scl}{(18.67,2.4)}{SCL}{above right}{SclWire}

  \ModulePort{supply-12v-in}{(-0.8,3.9)}{+12V IN}{right}{LogicPower}
  \ModulePort{supply-3v3-out}{(3.4,3.0)}{+3V3 OUT}{left}{LogicPower}
  \ModulePort{supply-gnd}{(1.0,2.3)}{GND}{above}{black!70}

  % --------------------------------------------------------------------------
  % Legend and construction warning.
  % --------------------------------------------------------------------------
  \draw[main current] (0.6,-3.25) -- (1.8,-3.25);
  \node[note, anchor=west] at (2.0,-3.25) {high-current conductor};
  \draw[sense] (5.3,-3.25) -- (6.5,-3.25);
  \node[note, anchor=west] at (6.7,-3.25) {Kelvin sense};
  \draw[i2c cable] (9.1,-3.36) -- (10.3,-3.36);
  \draw[sda] (9.1,-3.27) -- (10.3,-3.27);
  \draw[scl] (9.1,-3.45) -- (10.3,-3.45);
  \node[note, anchor=west] at (10.5,-3.36)
    {I\textsuperscript{2}C cable: SDA + SCL (2 nets)};
  \draw[logic power] (14.0,-3.25) -- (15.2,-3.25);
  \node[note, anchor=west] at (15.4,-3.25) {low-voltage power};

  \node[warning box, anchor=west] at (19.0,-3.35)
